...
//		bool isOwner;
	...
	//		if(!base.isLocalPlayer)return;
		...
		//	if(!base.isLocalPlayer)return;
            ...
            RaycastHit hitInfo;
            bool b = Physics.SphereCast(transform.position, m_CharacterController.radius, Vector3.down, out hitInfo,
                               m_CharacterController.height/2f);
            if(b&&hitInfo.transform.tag!="Bullet")desiredMove = Vector3.ProjectOnPlane(desiredMove, hitInfo.normal).normalized;
        ...
        void GetInput(out float speed)
        {
			...
			m_IsWalking = CrossPlatformInputManager.GetButton("Walk");
#endif
